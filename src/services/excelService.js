  ...
  if (typeof valor === "object") {
    ...
    for (const key of keys) {
      ...
      // Si es array
        ...
        if (
          subvalor.length <= 3 &&
          subvalor.every((v) => typeof v !== "object")
        ) {
...
const procesarPropuesta = (propuesta) => {
  ...
  if (propuesta.recomendaciones) {
    resultado.propuesta_recomendaciones = Array.isArray(
        ...
        .map((alt, i) => {
          ...
          if (typeof alt === "object" && alt !== null) {
            const nombre =
              alt.nombre ||
              alt.id ||
              objetoATextoLegible(alt, 0).split("\n")[0];
            return `${i + 1}. ${nombre}`;
          }
          return `${i + 1}. ${alt}`;
        })
  ...
  // Afinidades
  if (
    propuesta.afinidades &&
    Array.isArray(propuesta.afinidades) &&
  ...
  if (propuesta.paqueteRecomendado && propuesta.paqueteRecomendado.paquete) {
    ...
    if (typeof nombrePaquete === "object" && nombrePaquete !== null) {
      nombrePaquete =
        nombrePaquete.nombre || nombrePaquete.id || "No especificado";
    }
  ...
  if (
    propuesta.insights &&
    Array.isArray(propuesta.insights) &&
    propuesta.insights.length > 0
  ) {
  ...
  if (
    propuesta.recomendaciones &&
    Array.isArray(propuesta.recomendaciones) &&
    propuesta.recomendaciones.length > 0
  ) {
